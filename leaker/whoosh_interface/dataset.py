...
class WhooshKeywordQueryLog(KeywordQueryLog):
    ...
    remove_endstates: bool
        If True, remove keywords that result in end states in the transition matrix of the individual query sequences
    """
    ...
    def keywords_list(self, user_id: str = None, remove_endstates: bool = False) -> List[str]:
        if user_id is None:
            res = self.__keywords_list
        else:
            res = list(self.query(user_id))

        if remove_endstates:
            last = res[-1]
            while last not in res[:-2]:  # element last needs to appear one time before becoming the last element
                res = res[:-1]
                last = res[-1]
        return res
